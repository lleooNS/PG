...
Engineering}), diversas vantagens puderam ser aproveitadas durante a utilização de \textit{frameworks} na fase de projetos, fazendo com que o desenvolvimento de Sistemas de Informação baseados na Web (\textit{Web-based Information System – WISs}) se tornasse mais eficiente, proporcionando maior agilidade aos desenvolvedores. Na criação de aplicações para \textit{Web}, existem inúmeras categorias de \textit{frameworks} que podem ser escolhidas para a execução das implementações dos projetos. O método FrameWeb suporta algumas dessas categorias e por esse motivo existe a necessidade de realizar a verificação da eficiência, testando outros \textit{frameworks} de categorias que ainda não foram testadas.

A proposta do método vem sendo analisada através da aplicação para \textit{Web} SCAP (Sistema de Controle de Afastamento de Professores) e já foram utilizado alguns \textit{frameworks} para esse fim. Este trabalho tem como objetivo testar a eficácia do método FrameWeb, realizando uma nova implementação do SCAP, utilizando o \textit{framework} Grails para que seja acrescentado nas categorias suportadas.             
