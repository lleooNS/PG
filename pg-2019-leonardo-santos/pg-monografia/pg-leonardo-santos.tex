...
% Permite manter as figuras em suas seções.
\usepackage{placeins}
...
\newcommand{\vitor}[1]{\todo[author=\textbf{Professor},color=red!30,caption={},inline]{#1}}
\newcommand{\joao}[1]{\todo[author=\textbf{João Paulo},color=orange!30,caption={},inline]{#1}}
\newcommand{\pedro}[1]{\todo[author=\textbf{Pedro Henrique},color=purple!30,caption={},inline]{#1}}
...
sistema SCAP utilizando o framework Grails}
...
\begin{folhadeaprovacao}
  \begin{center}
    ...
    \vspace*{\fill}
   \end{center}
   Trabalho aprovado. \imprimirlocal, 14 de maio de 2021:
   \assinatura{\textbf{\imprimirorientador} \\ Orientador} 
   \assinatura{\textbf{Prof. Dr. João Paulo Andrade Almeida} \\ Universidade Federal do Espírito Santo}
   \assinatura{\textbf{Pedro Henrique Brunoro Hoppe} \\ Universidade Federal do Espírito Santo}
...
\begin{dedicatoria}
   ...
   \centering
   \noindent
   \textit{Dedico a Deus e aos meus pais que são tudo em minha vida.} \vspace*{\fill}
\end{dedicatoria}
...
\begin{epigrafe}
    \vspace*{\fill}
	\begin{flushright}
		\textit{``Buscai primeiro o Reino de Deus, e a sua justiça, \\
...
No desenvolvimento das primeiras aplicações para \textit{Web}, a preocupação em seguir a metodologia proposta pela Engenharia de Software não era muito relevante, fazendo com que estas aplicações fossem criadas de maneira \textit{ad-hoc}. Com a evolução e o crescimento da Internet, a construção de \textit{softwares} voltados para esse tipo de plataforma foi se tornando cada vez mais complexa, sendo necessário a adoção de técnicas que favorecessem a criação e adaptação das aplicações. Para esse fim, foi criada a Engenharia \textit{Web} (\textit{Web Engineering} ou \textit{WebE}).

Com o surgimento da proposta do método FrameWeb (\textit{Framework-based Design Method for Web
Engineering}), diversas vantagens puderam ser aproveitadas durante a utilização de \textit{frameworks} na fase de projetos, fazendo com que o desenvolvimento de Sistemas de Informação baseados na Web (\textit{Web-based Information System – WISs}) se tornasse mais eficiente, proporcionando maior agilidade aos desenvolvedores. 

Na criação de aplicações para \textit{Web}, existem inúmeras categorias de \textit{frameworks} que podem ser escolhidas para a execução das implementações dos projetos. O método FrameWeb se propõe a dar suporte a quatro dessas categorias (Controlador Frontal, Injeção de Dependências, Mapeamento Objeto/Relacional e Segurança) e, por esse motivo, existe a necessidade de realizar a verificação da eficácia deste suporte, testando \textit{frameworks} que fazem parte dessas categorias e verificando os resultados da aplicação do método.

A proposta do método vem sendo analisada por meio da aplicação \textit{Web} SCAP (Sistema de Controle de Afastamento de Professores) e já foram utilizado alguns \textit{frameworks} para esse fim. Este trabalho tem como objetivo testar a eficácia do método FrameWeb, realizando uma nova implementação do SCAP, utilizando o \textit{framework} Grails, dentro da categoria de Controlador Frontal.

\textbf{Palavras-chaves}: FrameWeb. Web. Engenharia Web. Grails.
\end{resumo}
...
\begin{siglas}
	...
	\item [CT] Centro Tecnológico
	\item [PRPPG] Pró-Reitoria de Pesquisa e Pós-Graduação
	\item [DAO] \textit{Data Access Object}
	\item [URL] \textit{Uniform Resource Locator}
	\item [GORM] \textit{Grails Object Relational Mapping}
	\item [API] \textit{Application Programming Interface}
	\item [SGBD] Sistema Gerenciador de Banco de Dados
	\item [CoC] \textit{Convention Over Configuration}
	\item [GSP] \textit{Groovy Server Pages}
...
\include{capitulos/ch2-referencial}
\include{capitulos/ch3-requisitos}
\include{capitulos/ch4-projeto}
\include{capitulos/ch5-conclusoes}
...
\includepdf[pages={1-}]{apendices/engsoft-especificacao-projeto-grails/projeto-scap-grails.pdf}
